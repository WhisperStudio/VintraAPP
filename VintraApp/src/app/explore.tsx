...
function AnalyticsBg() {
  ...
  return (
    <View pointerEvents="none" style={s.bgWrap}>
      <View style={s.bgBase} />
      <Animated.View style={[s.bgBlob1, blob1]} />
      <Animated.View style={[s.bgBlob2, blob2]} />
    </View>
  );
}
...
/* ── Metric card ──────────────────────────────────────────── */
function MetricCard({
  icon, iconBg, iconTint, title, value, sub, small,
}: {
  icon: any; iconBg: string; iconTint: string;
  title: string; value: string; sub: string; small?: boolean;
}) {
  return (
    <View style={s.metricCard}>
      ...
      <Text style={[s.metricValue, small && s.metricValueSm]}>{value}</Text>
      <Text style={s.metricSub} numberOfLines={2}>{sub}</Text>
    </View>
  );
}
  ...
  if (!total) {
    return (
      <View style={s.emptyBox}>
        <Text style={s.emptyText}>Ingen data for perioden</Text>
      </View>
    );
  }
  return (
    <View style={s.segWrap}>
      <View style={s.segTotalRow}>
        <Text style={s.segTotalNum}>{total}</Text>
        <Text style={s.segTotalLabel}> samtaler totalt</Text>
      </View>
      <View style={s.segBar}>
      ...
      <View style={s.segLegend}>
        {segments.map((sg, i) => (
          <View key={i} style={s.segLegendItem}>
            ...
            <Text style={s.segLegendLabel}>{sg.label}</Text>
            <Text style={s.segLegendCount}>{sg.value}</Text>
          </View>
        ))}
      </View>
    </View>
...
/* ── Timeline bar chart ───────────────────────────────────── */
function TimelineChart({ chats, days }: { chats: SupportChat[]; days: number }) {
            ...
            <View
              style={[
                s.timelineBar,
                {
                  ...
                  backgroundColor: b.count > 0 ? '#0f6eff' : 'rgba(255,255,255,0.05)',
                },
              ]}
            />
            {days <= 10 && <Text style={s.timelineLabel}>{b.label}</Text>}
          </View>
        ))}
      </View>
      {buckets.every(b => b.count === 0) && (
        <View style={s.emptyBox}>
          <Text style={s.emptyText}>Ingen aktivitet i perioden</Text>
        </View>
      )}
    </View>
  );
...
/* ── Country row ──────────────────────────────────────────── */
function CountryRow({ rank, code, count, max }: { rank?: number; code: string; count: number; max: number }) {
  return (
    <View style={s.countryRow}>
      {rank !== undefined && <Text style={s.countryRank}>#{rank}</Text>}
      <Text style={s.countryFlag}>{getFlag(code)}</Text>
      <Text style={s.countryCode}>{code}</Text>
      <View style={s.countryTrack}>
...
export default function AnalyticsScreen() {
  ...
  const chats = useMemo(() => {
    ...
    return allChats.filter(c => new Date(c.updatedAt) >= cutoff);
  }, [allChats, period, now]);

  const totalSessions = chats.length;
  const supportRequests = chats.filter(c => c.status === 'needs-human' || c.status === 'open').length;
  const aiOnly = chats.filter(c => c.status === 'ai-active').length;
  const savedChats = chats.filter(c => c.messages.some(m => m.role === 'support')).length;
      ...
      <ScrollView
        ...
        <View style={[s.metricGrid, !compact && s.metricGridWide]}>
          <MetricCard
            ...
            title={t('metric_total_sessions')} value={String(totalSessions)} sub={t('metric_only_selected')}
          />
          <MetricCard
        ...
        <View style={s.controlRow}>
            ...
            {(['oversikt', 'tidslinje', 'geografi'] as const).map(tabKey => (
              <Pressable
                ...
                <SymbolView
                  ...
                  size={11}
                  tintColor={tab === tabKey ? '#0f6eff' : '#475569'}
                />
          ...
          <View style={s.periodBar}>
            {(['7d', '30d', 'all'] as const).map(p => (
              <Pressable key={p} onPress={() => setPeriod(p)} style={[s.periodBtn, period === p && s.periodBtnActive]}>
        ...
        {tab === 'oversikt' && (
          <View style={[s.chartRow, !compact && s.chartRowWide]}>
            <View style={s.chartCard}>
              <Text style={s.chartTitle}>{t('chart_seg_title')}</Text>
              <Text style={s.chartSub}>{t('chart_seg_sub')}</Text>
              <SegmentBar
                segments={[
            ...
            <View style={s.chartCard}>
              <Text style={s.chartTitle}>{t('chart_countries_title')}</Text>
              <Text style={s.chartSub}>{t('chart_countries_sub')}</Text>
              {countryData.length === 0 ? (
                <View style={s.emptyBox}><Text style={s.emptyText}>{t('chart_no_geo')}</Text></View>
              ) : (
                countryData.slice(0, 7).map(([code, count]) => (
                  <CountryRow key={code} code={code} count={count} max={countryData[0][1]} />
                ))
              )}
            </View>
          </View>
        ...
        {tab === 'tidslinje' && (
          <View style={s.chartCard}>
            <Text style={s.chartTitle}>{t('chart_timeline_title')}</Text>
            <Text style={s.chartSub}>{t('chart_timeline_sub')}</Text>
            <TimelineChart chats={chats} days={timelineDays} />
          </View>
        )}
        ...
        {tab === 'geografi' && (
          <View style={s.chartCard}>
            <Text style={s.chartTitle}>{t('chart_geo_title')}</Text>
            <Text style={s.chartSub}>{t('chart_geo_sub')}</Text>
            {countryData.length === 0 ? (
              <View style={s.emptyBox}><Text style={s.emptyText}>{t('chart_no_geo')}</Text></View>
            ) : (
              countryData.map(([code, count], i) => (
                <CountryRow key={code} rank={i + 1} code={code} count={count} max={countryData[0][1]} />
              ))
            )}
          </View>
        )}
